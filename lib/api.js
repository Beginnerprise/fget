...
// Globals
const state = {
  totalOfBytesDownloadedFromServer: 0,
  totalSizeOfFile: 0,
  downloadStartEpoch: null,
  lastSpeedInBytes: 0,
  avgSpeedInBytes: 0,
  chunksCompleted: 0
};
 ...
 */
function _getRateWithUnits(bytes) {
  const sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB'];
  if (bytes === 0) {
    return '0 Byte';
  }
  const b = Math.log(bytes) / Math.log(1024);
  const i = parseInt(Math.floor(b), 10);
  const p = Math.pow(1024, i);
  const s = Math.round(bytes / p, 2);
  return `${s + sizes[i]}`;
}

function _doPurgeFile(file) {
  log.debug("_doPurgeFile", file);
  try {
    fs.unlinkSync(file);
  } catch (e) {
    log.debug("Deleting file failed:", file, e.toString());
  }
}
...
function _startChunk(outputFileName, options, chunkNumber, callback) {
  log.verbose(
    "_startChunk parms:",
    outputFileName,
    options,
  ...
  const chunkSize = Math.ceil(1 * Number(state.totalSizeOfFile) / api.chunkCount);
  // We will need to start writing this chunks bits at the right
  // offset inside the file
  const _offset = chunkSize * chunkNumber;
  ...
  // of the final file
  const _file = fs.createWriteStream(outputFileName, _fops);
  // Get the last byte location inside the final file
  const _endOfChunkBytes = Math.min(chunkSize * (chunkNumber + 1) - 1, state.totalSizeOfFile - 1);
...
var _downloadChunk = function(file, options, offset, end, chunkNumber, callback) {
  ...
  // Support for resume
  if (_resume && state.chunkStatus[chunkNumber]) {
    state.totalOfBytesDownloadedFromServer += state.chunkStatus[chunkNumber];
    offset += state.chunkStatus[chunkNumber];
  }
  ...
  var _request = request
    ...
    .on('data', function(data) {
      ...
      if (!state.chunkStatus[chunkNumber]) {
        state.chunkStatus[chunkNumber] = 0;
      }
...
var api = {
  ...
  get: function(options, callback) {
    ...
    api.filename = api.filename || decodeURI(_options.uri.replace(/\?.*/, "").split('/').pop());
    log.debug("Emitting to File:", api.filename);
    ...
    let _rootFileName = path.basename(api.filename);
    let _tmpStubFile = [".", _rootFileName, ".stub.fget.tmp"].join('');
    ...
    var _request = request
      ...
      .on('end', function() {
        ...
        _file.end(function() {
          ...
          fs.renameSync(_tmpStubFile, api.filename);
          return callback();
        });
      })
      .on('error', function(err) {
      ...
      .on('response', function(response) {
        ...
        // get files faster
        if (response.statusCode !== 206 || response.headers['content-length'] !== "1") {
          if (!response.headers['accept-ranges'] || !~response.headers['accept-ranges'].indexOf("bytes")) {
        ...
        // Build a temp file name we will dump data into until we complete.
        // Upon completition we'll move this temp file in place
        _rootFileName = path.basename(api.filename);
        let _tmpFileName = [".", _rootFileName, ".fget.tmp"].join('');

        // Open the file
        return fs.open(_tmpFileName, 'w', function(err, fd) {
          log.verbose("File Opened:", _tmpFileName);
          // Expand the file to size
          return fs.ftruncate(fd, state.totalSizeOfFile, function() {
            log.debug("File Truncated:", _tmpFileName);
            ...
            var _doHandleDownloadChunkDone = function(err) {
              ...
              if (state.chunksCompleted == api.chunkCount) {
                if (_cancel) {
                  ...
                  return fs.writeFileSync([_tmpFileName, ".resume"].join(''), JSON.stringify(state));
                  // return _doPurgeFile(_tmpFileName);
                }
                log.debug("Moving File Into Place");
                fs.renameSync(_tmpFileName, api.filename);
                _doPurgeFile([_tmpFileName, ".resume"].join(''));
            ...
            try {
              fs.statSync(_tmpFileName);
              var tmpResumeData = JSON.parse(fs.readFileSync([_tmpFileName, ".resume"].join(''), 'utf8'));
              if (tmpResumeData.totalSizeOfFile === state.totalSizeOfFile && api.chunkCount == tmpResumeData.chunksCompleted) {
                state.chunkStatus = tmpResumeData.chunkStatus;
                _resume = true;
                log.debug("Resume Enabled");
              }
            } catch (e) {
              // It is normal for an exception to be thrown so, don't do
              // anything but emit the error if someone wants it
              log.verbose("Resume Check Failed:", e.toString());
            }
