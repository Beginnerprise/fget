...
var api = {
    ...
    var _request = request
      ...
      .on('response', function(response) {
        ...
        if (response.statusCode !== 200 && response.statusCode !== 206) {
          api.cancel();
          log.warn("Non 200 response on head - Trying single chunk");
          return callback("Did not get a 200 response");
        }
        ...
        if (response.headers['content-range'] && ~response.headers['content-range'].indexOf("bytes")) {
          _totalSizeOfFile = response.headers['content-range'].split('/').pop();
        } else {
  ...
  // log.verbose(_request);
  // return _request.end();
  // var _request = request.get(options, function(err, response, body) {
  //
  //   // **********************
  //   // Guards
  //   // **********************
  ...
  //   }
  //
  //   log.debug("Response received:", options.uri);
  ...
  //       log.warn("Content Length information not in head request");
  //       _chunkCount = 1;
  //     }
  //
  //     // If the server doesn't support byte rangers we will not be able to
